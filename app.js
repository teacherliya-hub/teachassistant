    ...
    if (savedData) {
        try {
            ...
            classData.forEach(cls => {
                ...
                if (!cls.grouping) { // [NEW]
                    cls.grouping = { group_count: 0, groups: {}, scores: {} }; // scores added
                }
                if (!cls.grouping.scores) {
                    cls.grouping.scores = {};
                }
            });
...
function parseAndAddClass() {
    ...
    const newClass = {
        ...
        grouping: { group_count: 0, groups: {}, scores: {} } // [NEW]
    };
...
function renderGroupingView() {
    ...
    if (!currentClass.grouping) {
        currentClass.grouping = { group_count: 0, groups: {}, scores: {} };
        saveData();
    }
...
function renderGroupAreas() {
    ...
    const currentClass = classData[currentClassIndex];
    // 確保 scores 物件存在
    if (!currentClass.grouping.scores) {
        currentClass.grouping.scores = {};
    }

    const { group_count, groups, scores } = currentClass.grouping;
    ...
    for (let i = 1; i <= group_count; i++) {
        ...
        const currentGroupScore = scores[groupKey] || 0; // 取得目前小組分數
        
        const area = document.createElement('div');
        area.className = 'group-area relative';
        ...
        title.className = 'text-lg font-semibold text-blue-700 pb-2 border-b border-blue-200 text-center';
        title.textContent = `第 ${i} 組`;
        area.appendChild(title);

        // 2. [NEW] 小組加分控制區
        const scoreControl = document.createElement('div');
        scoreControl.className = 'flex items-center justify-center space-x-4 my-3 bg-blue-100/50 p-2 rounded-lg';
        scoreControl.innerHTML = `
            <button class="group-score-btn w-8 h-8 rounded-full bg-teal-500 text-white flex items-center justify-center hover:bg-teal-600 shadow transition focus:outline-none active:scale-95" 
                    data-group-key="${groupKey}" data-delta="-1" title="全組扣分">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd" />
                </svg>
            </button>
            <span id="group-score-display-${groupKey}" class="text-2xl font-bold text-slate-700 min-w-[2rem] text-center select-none">${currentGroupScore}</span>
            <button class="group-score-btn w-8 h-8 rounded-full bg-rose-500 text-white flex items-center justify-center hover:bg-rose-600 shadow transition focus:outline-none active:scale-95" 
                    data-group-key="${groupKey}" data-delta="1" title="全組加分">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" />
                </svg>
            </button>
        `;
        area.appendChild(scoreControl);
        
        // 3. 渲染組內學生
        const studentListContainer = document.createElement('div');
        studentListContainer.className = 'space-y-2';
        
        groupStudents.forEach(studentId => {
            const student = currentClass.students.find(s => s.id === studentId);
            if (student) {
                studentListContainer.appendChild(createStudentBlock(student, 'group'));
            } else {
                groups[groupKey] = groups[groupKey].filter(id => id !== studentId);
                saveData();
            }
        });
        area.appendChild(studentListContainer);
...
// [NEW] 處理小組加分
function handleGroupScoreUpdate(groupKey, delta) {
    if (currentClassIndex === -1) return;
    const currentClass = classData[currentClassIndex];
    
    // 初始化 scores 物件
    if (!currentClass.grouping.scores) currentClass.grouping.scores = {};
    if (!currentClass.grouping.scores[groupKey]) currentClass.grouping.scores[groupKey] = 0;

    // 1. 更新小組顯示分數
    currentClass.grouping.scores[groupKey] += delta;

    // 2. 更新該組所有學生的個人分數
    const groupStudentsIds = currentClass.grouping.groups[groupKey] || [];
    let updatedCount = 0;
    groupStudentsIds.forEach(studentId => {
        const student = currentClass.students.find(s => s.id === studentId);
        if (student) {
            student.score += delta;
            updatedCount++;
    ...
    // 更新小組分數顯示
    const scoreDisplay = document.getElementById(`group-score-display-${groupKey}`);
    if (scoreDisplay) {
        scoreDisplay.textContent = currentClass.grouping.scores[groupKey];
        scoreDisplay.classList.remove('scale-125', 'text-orange-500');
        void scoreDisplay.offsetWidth; // trigger reflow
        scoreDisplay.classList.add('scale-125', 'text-orange-500', 'transition-transform', 'duration-200');
        setTimeout(() => {
                scoreDisplay.classList.remove('scale-125', 'text-orange-500');
        }, 200);
    }
...
function handleGenerateGroupAreas() {
    ...
    const confirmAndGenerate = () => {
        // [MODIFIED] 初始化 scores
        currentClass.grouping = { group_count: newGroupCount, groups: {}, scores: {} }; 
        saveData();
        renderGroupingView();
...
function resetGrouping() {
    ...
    if (Object.keys(currentClass.grouping.groups).length === 0 || Object.values(currentClass.grouping.groups).every(arr => arr.length === 0)) {
        ...
        showConfirmationModal(
        '確認重新分組',
        '您確定要清空所有分組，讓全部學生回到「待分組」列表嗎？\n(注意：這也會清除目前的小組加分紀錄，但已加到學生的分數會保留)',
        () => {
            // 保留組數，但清空 groups 和 scores
            currentClass.grouping.groups = {};
            currentClass.grouping.scores = {}; // [MODIFIED]
            saveData();
            renderGroupingView();
            showMessage('分組與小組分數已清空。');
        }
    );
}
...
function handleDropOnGroup(e) {
    ...
    // 因為介面有加分區塊，直接用 appendChild 可能會破壞結構，這裡選擇重繪
    // 但為了拖曳的流暢性，可以優化。這裡先使用完整渲染確保正確性。
    renderGroupingView(); 
}
...
function saveEdit(classIndex, studentId, field, newValue) {
    ...
    if (student) {
        ...
        if (newValue === "") {
            ...
            if (isNaN(newId) || newId < 1) {
                ...
                student.id = newId;
                updateStudentIdInCharts(oldId, newId);
    ...
    if (didNameChange) {
        ...
        if (inputElement) {
            ...
            displayElement.id = `name-display-${studentId}`; 
    ...
    reader.onload = function(e) {
        try {
            ...
            const sanitizedData = importedData.map(cls => {
                ...
                // [NEW] 匯入分組與分數
                let grouping = { group_count: 0, groups: {}, scores: {} };
                if (cls.grouping && typeof cls.grouping === 'object') {
                    ...
                    if (typeof cls.grouping.scores === 'object') grouping.scores = cls.grouping.scores;
                }
...
window.onload = function() {
    ...
    // 3. [NEW] 分組頁面加分按鈕
    if (dom.groupingAreasContainer) {
        dom.groupingAreasContainer.addEventListener('click', (e) => {
            const btn = e.target.closest('.group-score-btn');
            if (!btn) return;
            ...
            if (groupKey && !isNaN(delta)) {
                handleGroupScoreUpdate(groupKey, delta);
            }
        });
    }
